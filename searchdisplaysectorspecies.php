<?php
    ...
    try{
            ...
            $query = "SELECT species.sectorID,species.speciesID,speciestype,animalname,sector,animaltype FROM species,animals,sector WHERE species.sectorID = :sectorID AND speciestype = :speciestype AND species.speciesID=animals.speciesID AND sector.sectorID = species.sectorID;";
    ...
    <div class="main">
        ...
        <form action="/ZooManagement/search.php" method="post" class="main-form2">
            <div class="main-right">
                ...
                {
                    $count = count($result);
                    echo '<div class="result-sectorID" align="center">Animals Which Are Present In '.$result[0]["sector"].' Sector And Which Are '.$result[0]["speciestype"].'</div>';
                        for($row = 0;$row<$count;$row++)
                        {
                            $index = $row+1;
                            echo '<div class="result-sectorID" align="center">'.$index.'. '.$result[$row]["animalname"].' - '.$result[$row]["animaltype"].'</div>';
                        }
                }
                else
