<?php
    ...
    try{
            ...
            $query = "SELECT DISTINCT speciestype,animaltype,animalname,sector FROM species,animals,sector WHERE species.sectorID = :sectorID AND species.speciesID = animals.speciesID and sector.sectorID = species.sectorID;";
        ...
        <form action="/ZooManagement/search.php" method="post" class="main-form2">
            <div class="main-right">
                ...
                <?php
                    if($result)
                    {
                        echo '<div class="result-sectorID" align="center">Animals Which Are Present In '.$result[0]["sector"].' Sector Are </div>';
                         for($row = 0;$row<count($result);$row++)
                         {
                            $index = $row+1;
                            echo '<div class="result-sectorID" align="center">'.$index.'. '.$result[$row]["animalname"].' - '.$result[$row]["speciestype"].'</div>';
                         }
                    }
                    else
